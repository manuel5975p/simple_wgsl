/*
 * bench_ours_matched.c - Our Vulkan cuFFT, matched sizes for comparison
 */
#include "cufft.h"
#include "cuda.h"
...
static void bench(int n, int batch) {
    ...
    int log2n = 0; { int tmp = n; while (tmp > 1) { log2n++; tmp >>= 1; } }
...
int main(void) {
    ...
    printf("Our cuFFT (Vulkan-backed), matched batches\n\n");
