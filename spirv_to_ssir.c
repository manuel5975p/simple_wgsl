            ...
            case SpvOpSpecConstantFalse:
                if (operand_count >= 2) {
                    ...
                    if (id < c->id_bound) {
                        ...
                        c->ids[id].constant.type_id = type_id;
                        c->ids[id].constant.is_composite = 0;
                        c->ids[id].constant.is_spec = ((SpvOp)opcode == SpvOpSpecConstantFalse);
                        c->ids[id].constant.value_count = 1;
                        c->ids[id].constant.values = (uint32_t *)SPIRV_TO_SSIR_MALLOC(sizeof(uint32_t));
                        if (c->ids[id].constant.values) {
                ...
                if (operand_count >= 2) {
                    ...
                    if (mode == SpvExecutionModeLocalSize && operand_count >= 5) {
                        if (c->pending_wg_count >= c->pending_wg_cap) {
                            ...
                            VtsPendingWorkgroupSize *tmp = (VtsPendingWorkgroupSize *)SPIRV_TO_SSIR_REALLOC(c->pending_wgs, ncap * sizeof(VtsPendingWorkgroupSize));
                            if (!tmp) break;
                            c->pending_wgs = tmp;
                    ...
                    if (mode == SpvExecutionModeDepthReplacing ||
                        ...
                        if (c->pending_exec_mode_count >= c->pending_exec_mode_cap) {
                            ...
                            VtsPendingExecMode *tmp = (VtsPendingExecMode *)SPIRV_TO_SSIR_REALLOC(
                                c->pending_exec_modes, ncap * sizeof(VtsPendingExecMode));
                            if (!tmp) break;
                            c->pending_exec_modes = tmp;
...
#define VTS_MAX_ID_BOUND (10 * 1024 * 1024)
    if (c.id_bound == 0 || c.id_bound > VTS_MAX_ID_BOUND) {
