...
static const int msl_reserved_count = (int)(sizeof(msl_reserved) / sizeof(msl_reserved[0]));
...
static void msl_set_name(MslCtx *c, uint32_t id, const char *n) {
    ...
    sw_set_name_escaped(c->id_names, c->id_names_cap, id, n,
                        msl_reserved, msl_reserved_count, sw_malloc_fn);
}
    ...
    switch (inst->op) {
        ...
        case SSIR_OP_ACCESS: {
            ...
            uint32_t cur_type_id = sw_resolve_access_base_type(
                ...
                (SsirInst *(*)(void *, uint32_t))msl_find_inst, (void *)c,
                c->current_func);
            for (uint16_t i = 0; i < inst->extra_count; i++) {
...
void ssir_to_msl_free(void *p) { STM_FREE(p); }
