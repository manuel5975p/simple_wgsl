...
typedef SwSpvIdKind SpvIdKind;
#define SPV_ID_UNKNOWN SW_SPV_ID_UNKNOWN
#define SPV_ID_TYPE SW_SPV_ID_TYPE
#define SPV_ID_CONSTANT SW_SPV_ID_CONSTANT
#define SPV_ID_VARIABLE SW_SPV_ID_VARIABLE
#define SPV_ID_FUNCTION SW_SPV_ID_FUNCTION
#define SPV_ID_LABEL SW_SPV_ID_LABEL
#define SPV_ID_INSTRUCTION SW_SPV_ID_INSTRUCTION
#define SPV_ID_EXT_INST_IMPORT SW_SPV_ID_EXT_INST_IMPORT

typedef SwSpvTypeKind SpvTypeKind;
#define SPV_TYPE_VOID SW_SPV_TYPE_VOID
#define SPV_TYPE_BOOL SW_SPV_TYPE_BOOL
#define SPV_TYPE_INT SW_SPV_TYPE_INT
#define SPV_TYPE_FLOAT SW_SPV_TYPE_FLOAT
#define SPV_TYPE_VECTOR SW_SPV_TYPE_VECTOR
#define SPV_TYPE_MATRIX SW_SPV_TYPE_MATRIX
#define SPV_TYPE_ARRAY SW_SPV_TYPE_ARRAY
#define SPV_TYPE_RUNTIME_ARRAY SW_SPV_TYPE_RUNTIME_ARRAY
#define SPV_TYPE_STRUCT SW_SPV_TYPE_STRUCT
#define SPV_TYPE_POINTER SW_SPV_TYPE_POINTER
#define SPV_TYPE_FUNCTION SW_SPV_TYPE_FUNCTION
#define SPV_TYPE_IMAGE SW_SPV_TYPE_IMAGE
#define SPV_TYPE_SAMPLED_IMAGE SW_SPV_TYPE_SAMPLED_IMAGE
#define SPV_TYPE_SAMPLER SW_SPV_TYPE_SAMPLER

typedef SwSpvTypeInfo SpvTypeInfo;
typedef SwSpvDecorationEntry SpvDecorationEntry;
typedef SwSpvMemberDecoration SpvMemberDecoration;
...
typedef SwSpvBasicBlock SpvBasicBlock;
typedef SwSpvFunction SpvFunction;
typedef SwStringBuffer StringBuffer;
...
#define sb_init sw_sb_init
#define sb_free sw_sb_free
#define wr_sb_append sw_sb_append
#define wr_sb_appendf sw_sb_appendf
#define sb_indent sw_sb_indent
#define sb_newline sw_sb_nl
...
WgslRaiser *wgsl_raise_create(const uint32_t *spirv, size_t word_count) {
    ...
    /* Sanity-check id_bound: SPIR-V allows sparse IDs, so id_bound can exceed
       word_count. Use a reasonable cap for DOS protection instead. */
#define WGSL_RAISE_MAX_ID_BOUND (10 * 1024 * 1024)
    if (r->id_bound == 0 || r->id_bound > WGSL_RAISE_MAX_ID_BOUND) {
        WGSL_FREE(r);
        return NULL;
    }
...
static SpvFunction *add_function(WgslRaiser *r) {
    SpvFunction *fn = sw_spv_add_function(
        &r->functions, &r->function_count, &r->function_cap, 0, realloc);
...
WgslRaiseResult wgsl_raise_parse(WgslRaiser *r) {
        ...
        switch ((SpvOp)opcode) {
            ...
            case SpvOpMemberName:
                if (operand_count >= 3) {
                    ...
                    if (name && struct_id < r->id_bound && member < 4096) {
                        SpvIdInfo *info = &r->ids[struct_id];
                ...
                if (operand_count >= 3) {
                    ...
                    if (name) {
                        if (fn_id < r->id_bound) {
                            ...
                            if (r->ids[fn_id].name) strcpy(r->ids[fn_id].name, name);
                        }
                        if (r->pending_ep_count >= r->pending_ep_cap) {
                            int ncap = r->pending_ep_cap ? r->pending_ep_cap * 2 : 4;
                            PendingEntryPoint *tmp = (PendingEntryPoint *)WGSL_REALLOC(r->pending_eps, ncap * sizeof(PendingEntryPoint));
                            if (!tmp) {
                                WGSL_FREE((void *)name);
                                break;
                            }
                        ...
                        if (ep->name) strcpy(ep->name, name);
                ...
                if (operand_count >= 2) {
                    ...
                    if (mode == SpvExecutionModeLocalSize && operand_count >= 5) {
                        if (r->pending_wg_count >= r->pending_wg_cap) {
                            ...
                            PendingWorkgroupSize *tmp = (PendingWorkgroupSize *)WGSL_REALLOC(r->pending_wgs, ncap * sizeof(PendingWorkgroupSize));
                            if (!tmp) break;
                            r->pending_wgs = tmp;
                ...
                if (operand_count >= 4) {
                    ...
                    if (id < r->id_bound) {
                        ...
                        if (r->ids[id].name) {
                            ...
                            if (current_fn->name) strcpy(current_fn->name, r->ids[id].name);
                        }
                    }
                    current_block = NULL;
                }
            ...
            case SpvOpFunctionParameter:
                if (current_fn && operand_count >= 2) {
                    uint32_t id = operands[1];
                    uint32_t *tmp = (uint32_t *)WGSL_REALLOC(current_fn->params, (current_fn->param_count + 1) * sizeof(uint32_t));
                    if (!tmp) break;
                    current_fn->params = tmp;
                    current_fn->params[current_fn->param_count++] = id;
                }
                break;
    ...
    for (int i = 0; i < r->function_count; i++) {
        ...
        if (fn_id < r->id_bound && r->ids[fn_id].name && !r->functions[i].name) {
            ...
            if (r->functions[i].name) strcpy(r->functions[i].name, r->ids[fn_id].name);
        }
    }
    ...
    for (int i = 0; i < r->pending_ep_count; i++) {
        ...
        for (int j = 0; j < r->function_count; j++) {
            if (r->functions[j].id == ep->func_id) {
                ...
                if (r->functions[j].name) strcpy(r->functions[j].name, ep->name);
...
// Inner function that takes a caller-provided buffer to avoid static buffer aliasing
// during recursion. Uses a local inner_buf for recursive calls.
static const char *type_to_wgsl_buf(WgslRaiser *r, uint32_t type_id, char *buf, size_t buf_size) {
    wgsl_compiler_assert(r != NULL, "type_to_wgsl_buf: r is NULL");
    ...
    switch (info->type_info.kind) {
        ...
        case SPV_TYPE_VECTOR: {
            ...
            char inner_buf[256]; // Local buffer for recursive call
            const char *ct = type_to_wgsl_buf(r, comp, inner_buf, sizeof(inner_buf));
            if (strcmp(ct, "f32") == 0) {
                snprintf(buf, buf_size, "vec%u<f32>", cnt);
            } else if (strcmp(ct, "i32") == 0) {
                snprintf(buf, buf_size, "vec%u<i32>", cnt);
            } else if (strcmp(ct, "u32") == 0) {
                snprintf(buf, buf_size, "vec%u<u32>", cnt);
            } else if (strcmp(ct, "bool") == 0) {
                snprintf(buf, buf_size, "vec%u<bool>", cnt);
            } else {
                snprintf(buf, buf_size, "vec%u<%s>", cnt, ct);
            }
            result = buf;
            break;
        }
        case SPV_TYPE_MATRIX: {
            ...
            if (col_type < r->id_bound && r->ids[col_type].kind == SPV_ID_TYPE &&
                ...
                char inner_buf[256]; // Local buffer for recursive call
                const char *et = type_to_wgsl_buf(r, elem, inner_buf, sizeof(inner_buf));
                if (strcmp(et, "f32") == 0) {
                    snprintf(buf, buf_size, "mat%ux%u<f32>", cols, rows);
                } else {
                    snprintf(buf, buf_size, "mat%ux%u<%s>", cols, rows, et);
                }
                result = buf;
            } else {
                result = "mat4x4<f32>";
        ...
        case SPV_TYPE_ARRAY: {
            ...
            if (len_id < r->id_bound && r->ids[len_id].kind == SPV_ID_CONSTANT) {
                ...
                snprintf(buf, buf_size, "array<%s, %u>", et, len);
            } else {
                snprintf(buf, buf_size, "array<%s>", et);
            }
            result = buf;
            break;
        }
        ...
        case SPV_TYPE_POINTER: {
            char inner_buf[256]; // Local buffer for recursive call
            const char *pt = type_to_wgsl_buf(r, info->type_info.pointer.pointee_type, inner_buf, sizeof(inner_buf));
            // Copy the result to our buffer since it may be in inner_buf
            snprintf(buf, buf_size, "%s", pt);
            result = buf;
            break;
        }
        case SPV_TYPE_IMAGE: {
        ...
        case SPV_TYPE_SAMPLED_IMAGE: {
            char inner_buf[256]; // Local buffer for recursive call
            const char *it = type_to_wgsl_buf(r, info->type_info.sampled_image.image_type, inner_buf, sizeof(inner_buf));
            // Copy the result to our buffer since it may be in inner_buf
            snprintf(buf, buf_size, "%s", it);
            result = buf;
            break;
        }
...
// Wrapper function that uses a static buffer for top-level calls only
// r nonnull
static const char *type_to_wgsl(WgslRaiser *r, uint32_t type_id) {
    static char buf[256];
    return type_to_wgsl_buf(r, type_id, buf, sizeof(buf));
}
...
static void wr_emit_constant(WgslRaiser *r, uint32_t id, StringBuffer *out) {
    ...
    SpvIdInfo *info = &r->ids[id];
    if (info->kind != SPV_ID_CONSTANT) {
        wr_sb_append(out, "0");
        return;
    }

    uint32_t type_id = info->constant.type_id;
    if (type_id >= r->id_bound) {
        wr_sb_append(out, "0");
        return;
    }
...
WgslRaiseResult wgsl_raise_to_wgsl(
    ...
    if (!r) {
        if (out_error) {
            *out_error = (char *)WGSL_MALLOC(32);
            if (*out_error) strcpy(*out_error, "Failed to create raiser");
        }
        return WGSL_RAISE_INVALID_SPIRV;
    }
    ...
    if (res != WGSL_RAISE_SUCCESS) {
        if (out_error && r->last_error[0]) {
            *out_error = (char *)WGSL_MALLOC(strlen(r->last_error) + 1);
            if (*out_error) strcpy(*out_error, r->last_error);
        }
    ...
    if (out_wgsl) {
        *out_wgsl = (char *)WGSL_MALLOC(strlen(wgsl) + 1);
        if (*out_wgsl) strcpy(*out_wgsl, wgsl);
    }
