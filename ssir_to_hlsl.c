...
#define STM_MALLOC  SW_MALLOC
#define STM_REALLOC SW_REALLOC
#define STM_FREE    SW_FREE
...
static const char *const hlsl_reserved[] = {
...
static const int hlsl_reserved_count = (int)(sizeof(hlsl_reserved) / sizeof(hlsl_reserved[0]));

static void hlsl_set_name(HlslCtx *c, uint32_t id, const char *n) {
    if (id >= c->id_names_cap) return;
    STM_FREE(c->id_names[id]);
    sw_set_name_escaped(c->id_names, c->id_names_cap, id, n,
                        hlsl_reserved, hlsl_reserved_count, sw_malloc_fn);
}
...
static void hlsl_emit_function(HlslCtx *c, SsirFunction *fn) {
    if (c->active_ep && c->active_ep->function == fn->id) {
        ...
        if (c->target_stage == SSIR_STAGE_COMPUTE) {
            ...
            if (c->active_ep) {
                x = c->active_ep->workgroup_size[0] ? c->active_ep->workgroup_size[0] : 1;
                y = c->active_ep->workgroup_size[1] ? c->active_ep->workgroup_size[1] : 1;
                z = c->active_ep->workgroup_size[2] ? c->active_ep->workgroup_size[2] : 1;
            }
    ...
    sw_emitter_build_maps(c->mod, fn, &c->use_counts, &c->inst_map, &c->inst_map_cap, sw_free_fn, sw_malloc_fn);
...
SsirToHlslResult ssir_to_hlsl(const SsirModule *mod,
    ...
    if (ctx.active_ep) {
        ...
        if (ctx.target_stage == SSIR_STAGE_COMPUTE) {
            uint32_t x = 1, y = 1, z = 1;
            if (ctx.active_ep) {
                x = ctx.active_ep->workgroup_size[0] ? ctx.active_ep->workgroup_size[0] : 1;
                y = ctx.active_ep->workgroup_size[1] ? ctx.active_ep->workgroup_size[1] : 1;
                z = ctx.active_ep->workgroup_size[2] ? ctx.active_ep->workgroup_size[2] : 1;
            }
            hb_appendf(&ctx.sb, "[numthreads(%u, %u, %u)]\n", x, y, z);
        }
...
void ssir_to_hlsl_free(void *p) { STM_FREE(p); }
